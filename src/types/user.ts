...
export const UserMethods = {
  is_guest: (user?: User | null) => !user || user.role === UserRole.GUEST,
  is_admin: (user?: User | null) => !!user && user.role === UserRole.ADMIN,
  is_general: (user?: User | null) => !!user && user.role === UserRole.GENERAL,
  can: (user: User | null | undefined, permission: number) => {
