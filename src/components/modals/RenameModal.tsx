...
import { fsRename } from '~/utils/api'
...
export const RenameModal: React.FC<RenameModalProps> = ({ target, isOpen, onClose }) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setLoading(true)
    const targetPath = (currentPath.endsWith('/') ? currentPath : currentPath + '/') + target.name
    try {
      ...
      if (resp.code === 200) {
        ...
        notify.error(resp.message || 'Failed to rename')
      }
    } catch (err: any) {
      notify.error(err.message || 'Failed to rename')
    } finally {
      setLoading(false)
    }
  }
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div className="flex justify-end space-x-2">
            ...
            >
              ...
              <span>{t('global.confirm') || 'Save'}</span>
            </button>
          </div>
        </form>
      </div>
