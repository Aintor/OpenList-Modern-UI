...
export const Manage: React.FC<ManageProps> = ({ onBackToFiles }) => {
  ...
  return (
    <div className="flex flex-1 overflow-hidden bg-slate-50 text-slate-900 dark:bg-slate-950 dark:text-slate-100">
      {/* Manage Left Sidebar Menu */}
      <aside className="flex w-64 flex-col justify-between border-r border-slate-200/80 bg-white/80 p-4 backdrop-blur-sm dark:border-slate-800/80 dark:bg-slate-900/80">
            ...
            {tabs.map((tab) => {
              ...
              return (
                <button
                  ...
                  className={`flex w-full items-center space-x-3 rounded-2xl px-3.5 py-2.5 text-xs font-semibold transition-all ${
                    isActive
